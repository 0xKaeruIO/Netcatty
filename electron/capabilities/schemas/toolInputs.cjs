...
const TOOL_INPUT_FIELDS = Object.freeze({
  ...
  "vault.host.update": {
    ...
    startupCommandRunMode: { type: "string", optional: true, description: "paste, lineDelay, or rules." },
    startupCommandRules: { type: "string", optional: true, description: "JSON array of {expect,send} steps for rule mode. Empty expect sends immediately. Empty array clears them." },
  ...
  "vault.host.import": {
    format: {
      type: "string",
      description: "Import format: csv, putty, mobaxterm, securecrt, xshell, ssh_config, or auto to detect from text.",
    },
    ...
    fileName: {
      type: "string",
      optional: true,
      description: "Optional source file name (helps SecureCRT .ini and Xshell .xsh parsing).",
    },
  },
...
const MODEL_DESCRIPTION_HINTS = Object.freeze({
  ...
  "vault.host.import":
    "Only for text in known export formats (PuTTY reg, MobaXterm ini, CSV template, SecureCRT, Xshell, ssh_config). If attached host text is unknown or auto-detection fails, use read_attachment content, extract fields yourself, and call vault_hosts_create.",
  "vault.hosts.create":
